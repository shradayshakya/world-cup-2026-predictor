...
- **Phase 6 (Polish): done.** Calibration log (PRD §10), probability-change arrows (§5.1a), mobile QA.
- `scripts/model/calibration.py`'s `score_resolved_matches()` reads the *previous* run's `matches.json` (same snapshot-before-overwrite trick as movers) and cross-references it against freshly scraped `results.json` to find matches that flipped from unplayed to played since the last run, scoring each with a 3-way Brier score and log-loss. `public/data/calibration_log.json` is the one daily file that's **append-only across the whole tournament** instead of overwritten — `merge_into_log()` dedups by `(date, home, away)` so a re-run never double-counts. Surfaced on the new `/about` page (summary stats + per-match table, linking back to each match).
- `scripts/model/changes.py` generalizes the day-over-day diffing pattern from movers (which only covered `won_tournament`) to every stage key in `probabilities.json`, plus per-match W/D/L in `matches.json`, suppressed below PRD §5.1a's 0.1pp jitter floor (tighter than movers' 0.5pp, since an arrow is a much lower-stakes claim than LLM prose) → `public/data/probability_changes.json`. `src/components/ChangeArrow.tsx` renders the ▲/▼, threaded into Group tables, Team detail, and Match detail.
- **Bracket-node occupancy arrows were explicitly scoped out**, not forgotten: a bracket slot's displayed probability comes from `bracket.json` (per-slot occupancy), a fourth distinct quantity from team-stage/match-outcome probabilities that nothing snapshots day-over-day. Adding that snapshot now wouldn't be verifiable until a second future run: revisit once `bracket.json` has a `previous_bracket` read of its own.
- **Real scraper bug found via today's live run, not caught by any prior test**: `scrapers/wikipedia.py`'s group-standings parser detected the host flag via `team.endswith("(H)")`. Once a group winner clinches the knockout stage early, Wikipedia's cell text becomes `"Canada (H, X)"` (host + qualified, comma-separated) instead of a bare `"(H)"` suffix — the check silently failed, leaving the literal mangled string as the "team name" in `groups.json`, which then `KeyError`'d in `simulate.py` since no match/Elo lookup recognized `"Canada (H, X)"`. Fixed by matching the whole trailing parenthetical (`TEAM_FLAGS_RE`) and splitting its comma-separated contents for the host check, regardless of order or what else is in there — future flag combinations (e.g. a 3rd annotation) are handled for free.
- Mobile QA found three pre-existing tables (Group standings, Squad, calibration's match-by-match) that clipped their rightmost columns at narrow viewports with no scroll affordance — the `1st`/`2nd`/`Adv.` columns (exactly the ones that just gained arrows) were invisible by default on mobile. Fixed by hiding the least-essential columns (`P`/`W`/`D`/`L`, `Caps`/`Goals`, `Brier`/`Log-loss`) below the `sm` breakpoint rather than shrinking text or removing the horizontal-scroll fallback.
- **Next concrete steps**: none currently planned — Phase 6 was the last phase in PRD §11. Future work would be reactive (recalibrating the model against more real results, a `previous_bracket` snapshot for bracket arrows, or whatever else surfaces from daily operation).
