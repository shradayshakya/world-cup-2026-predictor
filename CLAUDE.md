...
- `scripts/model/poisson.py` — Poisson-Elo + Dixon-Coles match model. **No historical results dataset was available to fit this**, so the Elo-to-goals mapping (`GOAL_SUPREMACY_PER_400_ELO = 2.5` — recalibrated 2026-06-18, see below; `AVERAGE_GOALS_PER_MATCH = 2.6`, `HOME_ADVANTAGE_ELO = 100`, `DIXON_COLES_RHO = -0.13`) is a documented heuristic, not a regression fit to match outcomes — revisit via the PRD §10 Brier-score calibration check once enough real matches have been played.
...
- **Recalibrated `GOAL_SUPREMACY_PER_400_ELO` from 1.0 → 2.5 (2026-06-18)**, prompted by the same user report: at 1.0 the model disagreed with eloratings.net's own win expectancy by ~12.6pp on average across the 48 group-stage matches, and 41 of 48 predicted scorelines collapsed to exactly 1-1 regardless of how lopsided the actual matchup was. Grid-searched against eloratings.net's win expectancy (free, already-scraped ground truth) — 2.5 cuts the disagreement to ~1.5pp. Note this mainly fixes *decisiveness* (a 90/10 mismatch now predicts 2-0 at ~89% rather than 1-1 at ~65%); it only modestly increases the *variety* of exact scorelines (still just {2-0, 1-1, 0-2} across today's 48 matches) since that's inherent to Poisson-mode behavior, not a calibration bug — the `TopScorelines` table above is the more honest fix for that part. Re-derive `matches.json`/`probabilities.json`/`bracket.json` from existing `elo.json`/`groups.json`/`results.json` after touching this constant — no need to re-scrape.
