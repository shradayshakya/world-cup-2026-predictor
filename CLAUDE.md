...
- **`elo_win_expectancy` is NOT a 3-way split** — it's eloratings.net's classic Elo "expected score" (win=1, draw=0.5, loss=0), not directly comparable to our `prediction`'s separate home/draw/away probabilities. Comparable single-number proxy from our side: `home_win_probability + 0.5 * draw_probability`. Cross-checked against our model: close matchups agree almost exactly (e.g. Switzerland vs Canada: 50% vs our 49.9%), but eloratings.net is consistently more confident in mismatches than our model (e.g. Spain vs Saudi Arabia: 96% vs our 78.9%) — a useful signal for the PRD §10 calibration check, since it suggests our `GOAL_SUPREMACY_PER_400_ELO` may be too conservative for large Elo gaps.
