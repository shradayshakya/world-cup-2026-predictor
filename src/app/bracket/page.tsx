...
import { getBracket, getResults } from "@/lib/data";
import { allTeamNames, matchSlug } from "@/lib/slug";
import type { BracketSlot, ResultMatch } from "@/lib/types";

function sideFor(name: string, slotSide: BracketSlot["home"], knownTeams: Set<string>): BracketSide {
  if (knownTeams.has(name)) {
    return { name, confirmed: true };
  }
  return { name: slotSide?.team ?? name, probability: slotSide?.probability, confirmed: false };
}
...
export default function BracketPage() {
  ...
  const rounds: Array<{ title: string; round: string; slots: BracketSlot[] }> = [
    { title: "Round of 32", round: "Round of 32", slots: bracket.round_of_32 },
    { title: "Round of 16", round: "Round of 16", slots: bracket.round_of_16 },
    { title: "Quarter-finals", round: "Quarter-finals", slots: bracket.quarter_finals },
    { title: "Semi-finals", round: "Semi-finals", slots: bracket.semi_finals },
  ];
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <div className="flex flex-wrap gap-10 overflow-x-auto">
        {rounds.map(({ title, round, slots }) => {
          const entries = roundEntries(allMatches, round);
          return (
            <BracketRound key={round} title={title}>
              {slots.map((slot, i) => {
                const entry = entries[i];
                const home = sideFor(entry.match.home_team, slot.home, knownTeams);
                const away = sideFor(entry.match.away_team, slot.away, knownTeams);
                return (
        ...
        <div className="flex flex-col gap-10">
          <BracketRound title="Final">
            {(() => {
              ...
              const home = sideFor(entry.match.home_team, bracket.final.home, knownTeams);
              const away = sideFor(entry.match.away_team, bracket.final.away, knownTeams);
          ...
          <BracketRound title="Third place play-off">
            {(() => {
              ...
              const home = sideFor(entry.match.home_team, bracket.third_place.home, knownTeams);
              const away = sideFor(entry.match.away_team, bracket.third_place.away, knownTeams);
              return <BracketNode home={home} away={away} href={`/matches/${matchSlug(entry.match, entry.index)}`} />;
            })()}
          </BracketRound>
        </div>
