...
- ~~**Movers commentary's day-over-day snapshot dependency**~~ — Locked: pulled forward a minimal slice of Phase 6 rather than deferring §6.4's movers commentary entirely. The mechanism turned out to be nearly free: `update.py` already overwrites `probabilities.json` each run, so reading the existing file *before* that write gives "yesterday's" data for free — no separate snapshot file, no git-history reads. Paired with a **0.5pp noise floor** on the day-over-day delta (more than double the ±0.2pp Monte Carlo jitter PRD §10 already documents), since treating re-simulation noise as a real movement worth an LLM "explanation" would itself be a kind of hallucination. First real run correctly produced zero movers (that day's "previous" snapshot was the same calibration just re-simulated, so the true delta was sub-jitter for every team) — confirms the floor is doing its job, not a bug.
